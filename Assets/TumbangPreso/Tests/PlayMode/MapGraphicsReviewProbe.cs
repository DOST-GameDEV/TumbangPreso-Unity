...
namespace TumbangPreso.PlayTests
{
    ...
    public sealed class MapGraphicsReviewProbe
    {
        ...
        {
            yield return MapRetrievalProbe.Load(SceneFlow.IlalimNgTulay);
            var room=GameObject.Find("IlalimNgTulay/Dressing/PlaceRework/Frontage_Print").transform;
            var face=room.Find("Sign face Print");var board=room.Find("Shop sign backing Print");Assert.IsNotNull(face);Assert.IsNotNull(board);
            var renderer=face.GetComponent<MeshRenderer>();var after=renderer.sharedMaterial;
            var saved=AssetDatabase.LoadAssetAtPath<Material>("Assets/TumbangPreso/Art/IlalimShopSigns/Print.mat");Assert.IsNotNull(saved);
            ...
            var equipment=room.Find("Print service equipment and awning");Assert.IsNotNull(equipment);Assert.IsEmpty(equipment.GetComponentsInChildren<Collider>());
            var oldParts=room.GetComponentsInChildren<MeshRenderer>().Where(r=>new[]{"Photocopier","Copier lid","Paper ream","Boxed shop stock","Glazed private shop boundary","Window frame"}.Contains(r.name)).ToArray();
            ...
            try
            {
                ...
                {
                    var target=room.TransformPoint(new Vector3(facePosition.x,2.92f,facePosition.z));
                    eye=view=="front"?target+room.forward*5:room.TransformPoint(new Vector3(3,1.7f,7));
                    rotation=Quaternion.LookRotation(target-eye);
                    foreach(string state in new[]{"before","after"})
                    {
                        ...
                        face.localPosition=on?facePosition:new Vector3(facePosition.x,2.92f,facePosition.z);
                        ...
                        using(Visual.NeighbourhoodSkyMotion.At(20))yield return GameplayShots.Render(camera,"Ilalim-print-sign-"+view+"-"+state,false,Output,width:1280,height:800);
                    }
                }
            }
        ...
        {
            yield return MapRetrievalProbe.Load(SceneFlow.IlalimNgTulay);
            var room=GameObject.Find("IlalimNgTulay/Dressing/PlaceRework/Frontage_Bakery").transform;
            var face=room.Find("Sign face Bakery");var board=room.Find("Shop sign backing Bakery");Assert.IsNotNull(face);Assert.IsNotNull(board);
            var renderer=face.GetComponent<MeshRenderer>();var after=renderer.sharedMaterial;
            var saved=AssetDatabase.LoadAssetAtPath<Material>("Assets/TumbangPreso/Art/IlalimShopSigns/Bakery.mat");Assert.IsNotNull(saved);
            var original=AssetDatabase.LoadAssetAtPath<Material>(saved.GetTag("TumpShopSignSource",false));Assert.IsNotNull(original);
            var texture=saved.mainTexture;var scale=face.localScale;var backing=board.localScale;
            var facePosition=face.localPosition;var boardPosition=board.localPosition;
            var equipment=room.Find("Bakery bread showcase");Assert.IsNotNull(equipment);Assert.IsEmpty(equipment.GetComponentsInChildren<Collider>());
            var oldParts=room.GetComponentsInChildren<MeshRenderer>().Where(r=>new[]{"Bread loaf","Food display tray","Boxed shop stock","Glazed private shop boundary","Window frame"}.Contains(r.name)).ToArray();
            Assert.IsTrue(oldParts.All(r=>!r.enabled));
            Assert.That(Mathf.Abs(face.lossyScale.x/face.lossyScale.y),Is.EqualTo(texture.width/(float)texture.height).Within(.002f));
            Assert.AreEqual(TextureImporterNPOTScale.None,((TextureImporter)AssetImporter.GetAtPath(AssetDatabase.GetAssetPath(texture))).npotScale);
            Assert.That(backing.y,Is.GreaterThan(scale.y));Assert.IsFalse(ShaderUtil.ShaderHasError(after.shader));
            ...
            try
            {
                foreach(string view in new[]{"front","street"})
                {
                    ...
                    foreach(string state in new[]{"before","after"})
                    {
                        ...
                        board.localPosition=on?boardPosition:new Vector3(boardPosition.x,2.92f,boardPosition.z);
                        equipment.gameObject.SetActive(on);foreach(var part in oldParts)part.enabled=!on;
                        using(Visual.NeighbourhoodSkyMotion.At(20))yield return GameplayShots.Render(camera,"Ilalim-bakery-"+view+"-"+state,false,Output,width:1280,height:800);
                    }
                }
            }
            finally{renderer.sharedMaterial=after;face.localScale=scale;board.localScale=backing;face.localPosition=facePosition;board.localPosition=boardPosition;equipment.gameObject.SetActive(true);foreach(var part in oldParts)part.enabled=false;Time.timeScale=1;Camera.onPreCull-=pin;if(clock!=null)clock.enabled=enabled;}
        ...
        {
            yield return MapRetrievalProbe.Load(SceneFlow.IlalimNgTulay);
            var room=GameObject.Find("IlalimNgTulay/Dressing/PlaceRework/Frontage_Pisonet").transform;
            var face=room.Find("Sign face Pisonet");var board=room.Find("Shop sign backing Pisonet");Assert.IsNotNull(face);Assert.IsNotNull(board);
            var kioskRenderers=room.Cast<Transform>().Where(t=>t.name.StartsWith("InteriorPisonet_")).SelectMany(t=>t.GetComponentsInChildren<MeshRenderer>()).ToArray();
            var kioskMaterials=kioskRenderers.Select(r=>r.sharedMaterials).ToArray();
            var oldScreen=AssetDatabase.LoadAllAssetsAtPath("Assets/TumbangPreso/Art/models/env_pisonet_kiosk.obj").OfType<Material>().Single(m=>m.name=="piso_screen");
            void Screens(bool afterState){for(int i=0;i<kioskRenderers.Length;i++)kioskRenderers[i].sharedMaterials=kioskMaterials[i].Select(m=>!afterState&&m.GetTag("TumpPisonetQuietScreen",false)=="1"?oldScreen:m).ToArray();}
            var renderer=face.GetComponent<MeshRenderer>();var after=renderer.sharedMaterial;
            var saved=AssetDatabase.LoadAssetAtPath<Material>("Assets/TumbangPreso/Art/IlalimShopSigns/Pisonet.mat");Assert.IsNotNull(saved);
            var original=AssetDatabase.LoadAssetAtPath<Material>(saved.GetTag("TumpShopSignSource",false));Assert.IsNotNull(original);
            var texture=saved.mainTexture;var scale=face.localScale;var backing=board.localScale;
            var facePosition=face.localPosition;var boardPosition=board.localPosition;
            var equipment=room.Find("Pisonet controls and lower glazing");Assert.IsNotNull(equipment);Assert.IsEmpty(equipment.GetComponentsInChildren<Collider>());
            var oldParts=room.GetComponentsInChildren<MeshRenderer>().Where(r=>new[]{"Shopfront lower wall"}.Contains(r.name)).ToArray();
            Assert.IsTrue(oldParts.All(r=>!r.enabled));
            Assert.That(Mathf.Abs(face.lossyScale.x/face.lossyScale.y),Is.EqualTo(texture.width/(float)texture.height).Within(.002f));
            Assert.AreEqual(TextureImporterNPOTScale.None,((TextureImporter)AssetImporter.GetAtPath(AssetDatabase.GetAssetPath(texture))).npotScale);
            Assert.That(backing.y,Is.GreaterThan(scale.y));Assert.IsFalse(ShaderUtil.ShaderHasError(after.shader));
            ...
            try
            {
                foreach(string view in new[]{"front","street"})
                {
                    ...
                    foreach(string state in new[]{"before","after"})
                    {
                        bool on=state=="after";Screens(on);renderer.sharedMaterial=on?after:original;face.localScale=on?scale:new Vector3(scale.x,scale.x*.25f,scale.z);
                        board.localScale=on?backing:new Vector3(backing.x,scale.x*.25f+.025f,backing.z);
                        face.localPosition=facePosition;
                        board.localPosition=on?boardPosition:new Vector3(boardPosition.x,2.92f,boardPosition.z);
                        equipment.gameObject.SetActive(on);foreach(var part in oldParts)part.enabled=!on;
                        using(Visual.NeighbourhoodSkyMotion.At(20))yield return GameplayShots.Render(camera,"Ilalim-pisonet-"+view+"-"+state,false,Output,width:1280,height:800);
                    }
                }
            }
        ...
        {
            yield return MapRetrievalProbe.Load(SceneFlow.IlalimNgTulay);
            var room=GameObject.Find("IlalimNgTulay/Dressing/PlaceRework/Frontage_Barber").transform;
            var face=room.Find("Sign face Barber");var board=room.Find("Shop sign backing Barber");Assert.IsNotNull(face);Assert.IsNotNull(board);
            var chairs=room.Cast<Transform>().Where(t=>t.name=="Barber chair").ToArray();
            var rotations=chairs.Select(t=>t.localRotation).ToArray();
            var renderer=face.GetComponent<MeshRenderer>();var after=renderer.sharedMaterial;
            var saved=AssetDatabase.LoadAssetAtPath<Material>("Assets/TumbangPreso/Art/IlalimShopSigns/Barber.mat");Assert.IsNotNull(saved);
            var original=AssetDatabase.LoadAssetAtPath<Material>(saved.GetTag("TumpShopSignSource",false));Assert.IsNotNull(original);
            var texture=saved.mainTexture;var scale=face.localScale;var backing=board.localScale;
            var facePosition=face.localPosition;var boardPosition=board.localPosition;
            var equipment=room.Find("Barber chair fittings and workstations");Assert.IsNotNull(equipment);Assert.IsEmpty(equipment.GetComponentsInChildren<Collider>());
            var oldParts=room.GetComponentsInChildren<MeshRenderer>().Where(r=>new[]{"Shopfront lower wall","Back shelf"}.Contains(r.name)).ToArray();
            Assert.IsTrue(oldParts.All(r=>!r.enabled));
            Assert.That(Mathf.Abs(face.lossyScale.x/face.lossyScale.y),Is.EqualTo(texture.width/(float)texture.height).Within(.002f));
            Assert.AreEqual(TextureImporterNPOTScale.None,((TextureImporter)AssetImporter.GetAtPath(AssetDatabase.GetAssetPath(texture))).npotScale);
            Assert.That(backing.y,Is.GreaterThan(scale.y));Assert.IsFalse(ShaderUtil.ShaderHasError(after.shader));
            ...
            try
            {
                foreach(string view in new[]{"front","street"})
                {
                    ...
                    foreach(string state in new[]{"before","after"})
                    {
                        bool on=state=="after";for(int i=0;i<chairs.Length;i++)chairs[i].localRotation=on?rotations[i]:Quaternion.identity;renderer.sharedMaterial=on?after:original;face.localScale=on?scale:new Vector3(scale.x,scale.x*.25f,scale.z);
                        board.localScale=on?backing:new Vector3(backing.x,scale.x*.25f+.025f,backing.z);
                        face.localPosition=facePosition;
                        board.localPosition=on?boardPosition:new Vector3(boardPosition.x,2.92f,boardPosition.z);
                        equipment.gameObject.SetActive(on);foreach(var part in oldParts)part.enabled=!on;
                        using(Visual.NeighbourhoodSkyMotion.At(20))yield return GameplayShots.Render(camera,"Ilalim-barber-"+view+"-"+state,false,Output,width:1280,height:800);
                    }
                }
            }
        ...
        {
            yield return MapRetrievalProbe.Load(SceneFlow.IlalimNgTulay);
            var room=GameObject.Find("IlalimNgTulay/Dressing/PlaceRework/Frontage_Clothing").transform;
            var face=room.Find("Sign face Clothing");var board=room.Find("Banner backing Clothing");Assert.IsNotNull(face);Assert.IsNotNull(board);
            var vendor=GameObject.Find("IlalimNgTulay/Dressing/PlaceRework/Vendor_ClothesAndCases").transform;
            var vendorPosition=vendor.position;
            // This pavement is inside the playable bounds. MapRouteProbe qualifies
            // real retrieval routes after the retained rack translation; x/shape are unchanged.
            var renderer=face.GetComponent<MeshRenderer>();var after=renderer.sharedMaterial;
            var saved=AssetDatabase.LoadAssetAtPath<Material>("Assets/TumbangPreso/Art/IlalimShopSigns/Clothing.mat");Assert.IsNotNull(saved);
            var original=AssetDatabase.LoadAssetAtPath<Material>(saved.GetTag("TumpShopSignSource",false));Assert.IsNotNull(original);
            var texture=saved.mainTexture;var scale=face.localScale;var backing=board.localScale;
            var facePosition=face.localPosition;var boardPosition=board.localPosition;
            var equipment=room.Find("Ciao Grazey shoe and garment display");Assert.IsNotNull(equipment);Assert.IsEmpty(equipment.GetComponentsInChildren<Collider>());
            var oldParts=room.GetComponentsInChildren<MeshRenderer>().Where(r=>new[]{"Folded stock","Back shelf","Window frame"}.Contains(r.name)).ToArray();
            Assert.IsTrue(oldParts.All(r=>!r.enabled));
            Assert.That(Mathf.Abs(face.lossyScale.x/face.lossyScale.y),Is.EqualTo(texture.width/(float)texture.height).Within(.002f));
            Assert.AreEqual(TextureImporterNPOTScale.None,((TextureImporter)AssetImporter.GetAtPath(AssetDatabase.GetAssetPath(texture))).npotScale);
            Assert.That(backing.y,Is.GreaterThan(scale.y));Assert.IsFalse(ShaderUtil.ShaderHasError(after.shader));
            ...
            try
            {
                foreach(string view in new[]{"front","street"})
                {
                    ...
                    foreach(string state in new[]{"before","after"})
                    {
                        bool on=state=="after";vendor.position=on?vendorPosition:new Vector3(vendorPosition.x,vendorPosition.y,-15.3f);renderer.sharedMaterial=on?after:original;face.localScale=on?scale:new Vector3(scale.x,scale.x*.28f,scale.z);
                        board.localScale=on?backing:new Vector3(backing.x,scale.x*.28f+.025f,backing.z);
                        face.localPosition=facePosition;
                        board.localPosition=on?boardPosition:new Vector3(boardPosition.x,2.92f,boardPosition.z);
                        equipment.gameObject.SetActive(on);foreach(var part in oldParts)part.enabled=!on;
                        using(Visual.NeighbourhoodSkyMotion.At(20))yield return GameplayShots.Render(camera,"Ilalim-ukay-"+view+"-"+state,false,Output,width:1280,height:800);
                    }
                }
            }
        ...
        {
            yield return MapRetrievalProbe.Load(SceneFlow.IlalimNgTulay);
            var room=GameObject.Find("IlalimNgTulay/Dressing/PlaceRework/Frontage_Load").transform;
            var face=room.Find("Sign face Load");var board=room.Find("Shop sign backing Load");Assert.IsNotNull(face);Assert.IsNotNull(board);
            var renderer=face.GetComponent<MeshRenderer>();var after=renderer.sharedMaterial;
            var saved=AssetDatabase.LoadAssetAtPath<Material>("Assets/TumbangPreso/Art/IlalimShopSigns/Load.mat");Assert.IsNotNull(saved);
            var original=AssetDatabase.LoadAssetAtPath<Material>(saved.GetTag("TumpShopSignSource",false));Assert.IsNotNull(original);
            var texture=saved.mainTexture;var scale=face.localScale;var backing=board.localScale;
            var facePosition=face.localPosition;var boardPosition=board.localPosition;
            var equipment=room.Find("Tindahan serving window and stock");Assert.IsNotNull(equipment);Assert.IsEmpty(equipment.GetComponentsInChildren<Collider>());
            var oldParts=room.GetComponentsInChildren<MeshRenderer>().Where(r=>new[]{"Boxed shop stock","Glazed private shop boundary","Window frame"}.Contains(r.name)).ToArray();
            Assert.IsTrue(oldParts.All(r=>!r.enabled));
            Assert.That(Mathf.Abs(face.lossyScale.x/face.lossyScale.y),Is.EqualTo(texture.width/(float)texture.height).Within(.002f));
            Assert.AreEqual(TextureImporterNPOTScale.None,((TextureImporter)AssetImporter.GetAtPath(AssetDatabase.GetAssetPath(texture))).npotScale);
            Assert.That(backing.y,Is.GreaterThan(scale.y));Assert.IsFalse(ShaderUtil.ShaderHasError(after.shader));
            ...
            try
            {
                foreach(string view in new[]{"front","street"})
                {
                    ...
                    foreach(string state in new[]{"before","after"})
                    {
                        ...
                        board.localPosition=on?boardPosition:new Vector3(boardPosition.x,2.92f,boardPosition.z);
                        equipment.gameObject.SetActive(on);foreach(var part in oldParts)part.enabled=!on;
                        using(Visual.NeighbourhoodSkyMotion.At(20))yield return GameplayShots.Render(camera,"Ilalim-tindahan-"+view+"-"+state,false,Output,width:1280,height:800);
                    }
                }
            }
            finally{renderer.sharedMaterial=after;face.localScale=scale;board.localScale=backing;face.localPosition=facePosition;board.localPosition=boardPosition;equipment.gameObject.SetActive(true);foreach(var part in oldParts)part.enabled=false;Time.timeScale=1;Camera.onPreCull-=pin;if(clock!=null)clock.enabled=enabled;}
        ...
        {
            yield return MapRetrievalProbe.Load(SceneFlow.IlalimNgTulay);
            var room=GameObject.Find("IlalimNgTulay/Dressing/PlaceRework/Frontage_PC_Express").transform;
            var face=room.Find("Sign face PC_Express");var board=room.Find("Shop sign backing PC_Express");Assert.IsNotNull(face);Assert.IsNotNull(board);
            var renderer=face.GetComponent<MeshRenderer>();var after=renderer.sharedMaterial;
            var saved=AssetDatabase.LoadAssetAtPath<Material>("Assets/TumbangPreso/Art/IlalimShopSigns/PC_Express.mat");Assert.IsNotNull(saved);
            var original=AssetDatabase.LoadAssetAtPath<Material>(saved.GetTag("TumpShopSignSource",false));Assert.IsNotNull(original);
            var texture=saved.mainTexture;var scale=face.localScale;var backing=board.localScale;
            var facePosition=face.localPosition;var boardPosition=board.localPosition;
            CollectionAssert.AreEqual(File.ReadAllBytes("Assets/TumbangPreso/Art/models/textures/pc_express_horizontal_rgb.png"),File.ReadAllBytes("Assets/TumbangPreso/Art/IlalimShopSigns/PCExpress-preserved.png"));
            var sourceImporter=(TextureImporter)AssetImporter.GetAtPath(AssetDatabase.GetAssetPath(texture));
            sourceImporter.GetSourceTextureWidthAndHeight(out int sourceWidth,out int sourceHeight);
            Assert.That(Mathf.Abs(face.lossyScale.x/face.lossyScale.y),Is.EqualTo(sourceWidth/(float)sourceHeight).Within(.002f));
            Assert.AreEqual(TextureImporterNPOTScale.None,((TextureImporter)AssetImporter.GetAtPath(AssetDatabase.GetAssetPath(texture))).npotScale);
            Assert.That(backing.y,Is.GreaterThan(scale.y));Assert.IsFalse(ShaderUtil.ShaderHasError(after.shader));
            var rig=Object.FindFirstObjectByType<CameraRig>();rig.enabled=false;var camera=rig.Camera;
            foreach(var arms in Object.FindObjectsByType<ViewmodelArms>(FindObjectsSortMode.None))arms.gameObject.SetActive(false);
            ...
            try
            {
                foreach(string view in new[]{"front","street"})
                {
                    ...
                    foreach(string state in new[]{"before","after"})
                    {
                        ...
                        face.localPosition=facePosition;
                        board.localPosition=on?boardPosition:new Vector3(boardPosition.x,2.92f,boardPosition.z);
                        using(Visual.NeighbourhoodSkyMotion.At(20))yield return GameplayShots.Render(camera,"Ilalim-pcexpress-"+view+"-"+state,false,Output,width:1280,height:800);
                    }
                }
            }
        ...
        {
            yield return MapRetrievalProbe.Load(SceneFlow.IlalimNgTulay);
            var room=GameObject.Find("IlalimNgTulay/Dressing/PlaceRework/Frontage_Repair").transform;
            var face=room.Find("Sign face Repair");var board=room.Find("Shop sign backing Repair");Assert.IsNotNull(face);Assert.IsNotNull(board);
            var renderer=face.GetComponent<MeshRenderer>();var after=renderer.sharedMaterial;
            var saved=AssetDatabase.LoadAssetAtPath<Material>("Assets/TumbangPreso/Art/IlalimShopSigns/Repair.mat");Assert.IsNotNull(saved);
            var original=AssetDatabase.LoadAssetAtPath<Material>(saved.GetTag("TumpShopSignSource",false));Assert.IsNotNull(original);
            var texture=saved.mainTexture;var scale=face.localScale;var backing=board.localScale;
            var facePosition=face.localPosition;var boardPosition=board.localPosition;
            var equipment=room.Find("Computer repair workbench");Assert.IsNotNull(equipment);Assert.IsEmpty(equipment.GetComponentsInChildren<Collider>());
            var oldParts=room.GetComponentsInChildren<MeshRenderer>().Where(r=>new[]{"Display monitor","Boxed shop stock","Glazed private shop boundary","Window frame"}.Contains(r.name)).ToArray();
            Assert.IsTrue(oldParts.All(r=>!r.enabled));
            Assert.That(Mathf.Abs(face.lossyScale.x/face.lossyScale.y),Is.EqualTo(texture.width/(float)texture.height).Within(.002f));
            Assert.AreEqual(TextureImporterNPOTScale.None,((TextureImporter)AssetImporter.GetAtPath(AssetDatabase.GetAssetPath(texture))).npotScale);
            Assert.That(backing.y,Is.GreaterThan(scale.y));Assert.IsFalse(ShaderUtil.ShaderHasError(after.shader));
            ...
            try
            {
                foreach(string view in new[]{"front","street"})
                {
                    ...
                    foreach(string state in new[]{"before","after"})
                    {
                        ...
                        board.localPosition=on?boardPosition:new Vector3(boardPosition.x,2.92f,boardPosition.z);
                        equipment.gameObject.SetActive(on);foreach(var part in oldParts)part.enabled=!on;
                        using(Visual.NeighbourhoodSkyMotion.At(20))yield return GameplayShots.Render(camera,"Ilalim-repair-"+view+"-"+state,false,Output,width:1280,height:800);
                    }
                }
            }
            finally{renderer.sharedMaterial=after;face.localScale=scale;board.localScale=backing;face.localPosition=facePosition;board.localPosition=boardPosition;equipment.gameObject.SetActive(true);foreach(var part in oldParts)part.enabled=false;Time.timeScale=1;Camera.onPreCull-=pin;if(clock!=null)clock.enabled=enabled;}
        ...
        {
            yield return MapRetrievalProbe.Load(SceneFlow.IlalimNgTulay);
            var room=GameObject.Find("IlalimNgTulay/Dressing/PlaceRework/Frontage_Laundry").transform;
            var face=room.Find("Sign face Laundry");var board=room.Find("Banner backing Laundry");Assert.IsNotNull(face);Assert.IsNotNull(board);
            var renderer=face.GetComponent<MeshRenderer>();var after=renderer.sharedMaterial;
            var saved=AssetDatabase.LoadAssetAtPath<Material>("Assets/TumbangPreso/Art/IlalimShopSigns/Laundry.mat");Assert.IsNotNull(saved);
            var original=AssetDatabase.LoadAssetAtPath<Material>(saved.GetTag("TumpShopSignSource",false));Assert.IsNotNull(original);
            var texture=saved.mainTexture;var scale=face.localScale;var backing=board.localScale;
            var facePosition=face.localPosition;var boardPosition=board.localPosition;
            var equipment=room.Find("Laundry service fittings");Assert.IsNotNull(equipment);Assert.IsEmpty(equipment.GetComponentsInChildren<Collider>());
            var oldParts=room.GetComponentsInChildren<MeshRenderer>().Where(r=>new[]{"Folded laundry","Shopfront lower wall"}.Contains(r.name)).ToArray();
            Assert.IsTrue(oldParts.All(r=>!r.enabled));
            Assert.That(Mathf.Abs(face.lossyScale.x/face.lossyScale.y),Is.EqualTo(texture.width/(float)texture.height).Within(.002f));
            Assert.AreEqual(TextureImporterNPOTScale.None,((TextureImporter)AssetImporter.GetAtPath(AssetDatabase.GetAssetPath(texture))).npotScale);
            Assert.That(backing.y,Is.GreaterThan(scale.y));Assert.IsFalse(ShaderUtil.ShaderHasError(after.shader));
            ...
            try
            {
                foreach(string view in new[]{"front","street"})
                {
                    ...
                    foreach(string state in new[]{"before","after"})
                    {
                        bool on=state=="after";renderer.sharedMaterial=on?after:original;face.localScale=on?scale:new Vector3(scale.x,scale.x*.27f,scale.z);
                        board.localScale=on?backing:new Vector3(backing.x,scale.x*.27f+.025f,backing.z);
                        face.localPosition=facePosition;
                        board.localPosition=on?boardPosition:new Vector3(boardPosition.x,2.97f,boardPosition.z);
                        equipment.gameObject.SetActive(on);foreach(var part in oldParts)part.enabled=!on;
                        using(Visual.NeighbourhoodSkyMotion.At(20))yield return GameplayShots.Render(camera,"Ilalim-laundry-"+view+"-"+state,false,Output,width:1280,height:800);
                    }
                }
            }
            finally{renderer.sharedMaterial=after;face.localScale=scale;board.localScale=backing;face.localPosition=facePosition;board.localPosition=boardPosition;equipment.gameObject.SetActive(true);foreach(var part in oldParts)part.enabled=false;Time.timeScale=1;Camera.onPreCull-=pin;if(clock!=null)clock.enabled=enabled;}
        ...
        [UnityTest,Timeout(90000)]
        public IEnumerator EskinitaWestAuthoredSignReview()
        {
            yield return MapRetrievalProbe.Load(SceneFlow.Eskinita);
            var shop=GameObject.Find("Eskinita/Dressing/Kalat/SariSari_W").transform;
            ...
            try
            {
                foreach(string view in new[]{"front","neighborhood"})
                {
                    ...
                    foreach(string state in new[]{"before","after"})
                    {
                        ...
                        using(Visual.NeighbourhoodSkyMotion.At(20))yield return GameplayShots.Render(camera,"Eskinita-west-sign-"+view+"-"+state,false,Output,width:1280,height:800);
                    }
                }
            }
