...
{
    public static class EskinitaDogHabitatAuthor
    {
        public static void Run()
        {
            ...
            Directory.CreateDirectory("Logs/eskinita-dog");File.WriteAllText("Logs/eskinita-dog/author.txt",report.ToString());
        ...
        public static void FinishLoadedScene(StringBuilder report)
        {
            ...
            var animal=life.Animals.Single(a=>a.Id=="aspin-tan");
            using(var geometry=new AmbientLifeAuthor.SurfaceQueries())
            {
                ...
                Site("South court watch",south,AmbientLife.Activity.Watch,new Vector3(0,south.y,south.z));
                Site("North verge investigation",north,AmbientLife.Activity.Investigate,north+Vector3.left);
                if(sites.Count<3)throw new InvalidOperationException("Dog habitat needs distinct usable activities");
                animal.Activities=sites.ToArray();
                report.AppendLine("Eskinita aspin-tan only: "+animal.Habitat.Length+" supported nodes, "+animal.Habitat.Sum(n=>n.Links.Length)/2+" clear links.");
