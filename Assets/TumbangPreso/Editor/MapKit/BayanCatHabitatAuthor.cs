...
{
    public static class BayanCatHabitatAuthor
    {
        public static void Run()
        {
            ...
            Directory.CreateDirectory("Logs/bayan-cat");File.WriteAllText("Logs/bayan-cat/author.txt",report.ToString());
        ...
        public static void FinishLoadedScene(StringBuilder report)
        {
            ...
            var animal=life.Animals.Single(a=>a.Id=="pusakal-ginger");
            using(var geometry=new AmbientLifeAuthor.SurfaceQueries())
            {
                ...
                Site("Paving seam investigation",south,AmbientLife.Activity.Investigate,south+Vector3.forward);
                Site("North plaza watch",north,AmbientLife.Activity.Watch,new Vector3(0,north.y,north.z));
                var corner=ordered[0].Point;
                Site("South corner watch",corner,AmbientLife.Activity.Watch,new Vector3(0,corner.y,corner.z));
                if(sites.Count<3)throw new InvalidOperationException("Cat habitat needs distinct usable activities");
                foreach(var site in sites)if(site.Kind==AmbientLife.Activity.Watch)site.HoldSeconds=site.Name=="North plaza watch"?new Vector2(10,16):new Vector2(6,10);
                animal.Activities=sites.ToArray();animal.QuietCat=true;animal.WalkSpeed=.18f;animal.RunSpeed=1.10f;
                report.AppendLine("BayanPlaza pusakal-ginger only: "+animal.Habitat.Length+" supported nodes, "+animal.Habitat.Sum(n=>n.Links.Length)/2+" clear links.");
