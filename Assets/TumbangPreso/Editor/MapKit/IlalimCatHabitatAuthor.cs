...
{
    public static class IlalimCatHabitatAuthor
    {
        ...
        public static void FinishLoadedScene(StringBuilder report)
        {
            ...
            var animal=life.Animals.Single(a=>a.Id=="pusakal-tuxedo");
            using(var geometry=new AmbientLifeAuthor.SurfaceQueries())
            {
                ...
                var pisonet=map.transform.Find("Dressing/PlaceRework/Frontage_Pisonet");
                var bakery=map.transform.Find("Dressing/PlaceRework/Frontage_Bakery");
                if(pisonet==null||bakery==null)throw new InvalidOperationException("Tuxedo cat needs its retained storefront references");
                Site("Pisonet-side watch",pisonet.position+Vector3.left*1.7f,AmbientLife.Activity.Watch,pisonet.position);
                Site("Bakeryward watch",bakery.position+Vector3.left*1.7f,AmbientLife.Activity.Watch,bakery.position);
                var ordered=animal.Habitat.OrderBy(n=>n.Point.z).ToArray();
                var seam=ordered[ordered.Length/4].Point;
                Site("Pavement seam investigation",seam,AmbientLife.Activity.Investigate,seam+Vector3.forward);
                if(sites.Count<3)throw new InvalidOperationException("Cat habitat needs distinct usable activities");
                foreach(var site in sites)if(site.Kind==AmbientLife.Activity.Watch)site.HoldSeconds=site.Name=="Pisonet-side watch"?new Vector2(7,12):new Vector2(4,8);
                animal.Activities=sites.ToArray();animal.QuietCat=true;animal.WalkSpeed=.19f;animal.RunSpeed=1.10f;
                report.AppendLine("IlalimNgTulay pusakal-tuxedo only: "+animal.Habitat.Length+" supported nodes, "+animal.Habitat.Sum(n=>n.Links.Length)/2+" clear links.");
