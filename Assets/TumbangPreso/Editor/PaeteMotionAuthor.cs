...
{
    // Paete's authored cast clips, baked from the rig on every roster build (HERO-9, 2026-09-26).
    //
    ...
    // header). `AmihanMotionAuthor` is the precedent this copies. ⚠️ AND FROM THE RIG, NOT THE MESH:
    // the owner's verdict on model v17 is still owed, so a rebuilt glb re-bakes these from
    // `HeroAbilityClips.Paete.cs` with no change here.
    public static class PaeteMotionAuthor
    {
        private const string Folder="Assets/TumbangPreso/Art/characters/paete-motion";
        ...
        public static AnimationClip[] Bake(GameObject model)
        {
            ...
            var copy=Object.Instantiate(model);copy.name="Paete motion authoring copy";
            ...
            try
            {
                ...
                generated=HeroAbilityClips.BuildPaeteAuthored(root);
