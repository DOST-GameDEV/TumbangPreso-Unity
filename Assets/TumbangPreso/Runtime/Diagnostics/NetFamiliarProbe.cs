...
namespace TumbangPreso.Diagnostics
{
    ...
    public sealed class NetFamiliarProbe : MonoBehaviour
    {
        ...
        private static void Install()
        {
            ...
            probe._writer.WriteLine("time,elapsed,host,local,possessed,devouring,x,y,z,bodyX,bodyZ,fieldX,fieldZ,fieldCount,s2active,ultactive,s2charges,ultcharge,modelX,modelZ");
        }
        private void Update()
        {
            if(!Active){enabled=false;return;}
            var round=GameServices.Round;
            if(!NetAuthority.IsNetworked || round==null || !round.RoundActive)return;
            ...
            foreach(var motor in round.Players)
                if(motor!=null){motor.Intent.Move=Vector2.zero;motor.Intent.Parked=motor!=_who;}
            if(!_prepared)
            {
                ...
                if(_scenario!="observe" && (NetAuthority.IsHost || NetAuthority.LocalSlot==1))_who.Teleport(new Vector3(0,.12f,-8));
            ...
            if(pet==null)return;
            if(NetAuthority.LocalSlot==1 && _scenario!="observe")
            {
