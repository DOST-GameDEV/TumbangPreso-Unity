...
namespace TumbangPreso.Diagnostics
{
    ...
    public sealed class NetZackProbe : MonoBehaviour
    {
        ...
        private void Update()
        {
            ...
            if (caster == null || front == null || !(caster.AbilitySystem?.Kit is ZackHeroKit kit)) return;
            ...
            if (!_prepared)
            {
                ...
                if (NetAuthority.IsHost)
                {
                    if (_scenario == "magnet")
                    {
                        _shoe = caster.GetComponent<Carrier>().Held;
                        if (_shoe != null)
                        {
                            _shoe.HostDisarm();
                            var from = new Vector3(1, 0, -4); from.y = Slipper.GroundY(from) + _shoe.RestHeight;
                            _shoe.transform.position = from;
                        }
                    }
            ...
            var bolts = FindObjectsByType<DirectedLightningBolt>(FindObjectsSortMode.None);
            var bolt = bolts.Length > 0 ? bolts[0].transform.position : Vector3.zero;
            ...
            object[] row = { now, elapsed, NetAuthority.LocalSlot, NetAuthority.IsHost ? 1 : 0,
                kit.HeroId == "zack" ? 1 : 0, caster.CharacterIndex, kit.IsOverchargeThrowActive ? 1 : 0,
