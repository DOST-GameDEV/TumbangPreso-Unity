...
namespace TumbangPreso.Diagnostics
{
    ...
    public sealed class NetCatchPresentationProbe : MonoBehaviour
    {
        ...
        private static void Install()
        {
            ...
            probe._writer.WriteLine("real,elapsed,local,playing,stun,tayaCanAct,tags,tayaX,tayaZ,victimX,victimZ,score");
        }
        ...
        private void Update()
        {
            ...
            if (!_prepared)
            {
                ...
                if (NetAuthority.IsHost || local == 1)
            ...
            if (NetAuthority.IsHost && !_punched && elapsed >= 8)
            {
                _punched = true;
                bool accepted = taya.GetComponent<CombatVerbs>().HostResolvePunch(taya.transform.position, Vector3.forward);
                Debug.Log("[CatchPeer] host punch accepted=" + accepted + " victimStun=" + victim.StunLeft);
            }
            if (NetAuthority.IsHost && elapsed >= 8.1f && elapsed < 8.8f) taya.Intent.Move = Vector2.up;
            ...
            _writer.WriteLine(FormattableString.Invariant($"{Time.realtimeSinceStartup-_started:F4},{elapsed:F4},{local},{(playing?1:0)},{victim.StunLeft:F4},{(taya.CanAct()?1:0)},{_tags},{a.x:F4},{a.z:F4},{b.x:F4},{b.z:F4},{match.ScoreFor(0)}"));
        }
