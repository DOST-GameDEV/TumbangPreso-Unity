...
namespace TumbangPreso.UI
{
    ...
    public sealed class OwnerScoreStrip : MaskableGraphic
    {
        ...
        protected override void OnPopulateMesh(VertexHelper helper)
        {
            ...
            {
                int first=helper.currentVertCount;
                helper.AddVert(new Vector2(rect.xMin+5,rect.yMin+3),CourtPresentationPalette.Gold,Vector2.zero);
                helper.AddVert(new Vector2(rect.xMax-6,rect.yMin+8),CourtPresentationPalette.Gold,Vector2.zero);
                helper.AddVert(new Vector2(rect.xMax-6,rect.yMin+11),CourtPresentationPalette.Gold,Vector2.zero);
                helper.AddVert(new Vector2(rect.xMin+5,rect.yMin+7),CourtPresentationPalette.Gold,Vector2.zero);
                helper.AddTriangle(first,first+1,first+2);helper.AddTriangle(first,first+2,first+3);
            }
        }
    }
