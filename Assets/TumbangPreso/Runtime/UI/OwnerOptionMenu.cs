...
{
    public sealed class OwnerOptionMenu : MonoBehaviour
    {
        public static OwnerOptionMenu OpenOption { get; private set; }
        ...
        public static OwnerOptionMenu Create(Transform parent,string name,string[] values,int selected,Action<int> choose)
        {
            var frame=OwnerUiLayout.Art(parent,name,OwnerUiTheme.Piece.FirstField);frame.raycastTarget=true;
            var menu=frame.gameObject.AddComponent<OwnerOptionMenu>();menu._options=values;
            menu._selected=Mathf.Clamp(selected,0,Mathf.Max(0,values.Length-1));menu._choose=choose;
            menu.Button=frame.gameObject.AddComponent<OwnerTextAction>();menu.Button.targetGraphic=frame;
            menu.Button.transition=Selectable.Transition.None;menu.Button.onClick.AddListener(menu.Open);
            menu._value=OwnerUiLayout.Text(frame.transform,"SelectedValue",values.Length>0?values[menu._selected]:"",29);
            menu._value.color=OwnerUiTheme.Current.EnteredInk;OwnerUiLayout.Place(menu._value.rectTransform,25,8,428,60);
            var mark=OwnerUiGlyph.Create(frame.transform,"Expand",OwnerUiGlyph.Mark.Back,OwnerUiTheme.Current.ActionInk);
        ...
        private void Open()
        {
            ...
            var paper=box.gameObject.AddComponent<OwnerUiPaper>();paper.raycastTarget=true;
            var column=OwnerScrollColumn.Build(box,"ChoiceList",new Rect(22,20,522,height-40),out var scroll);
            Button selectedButton=null;
            for(int i=0;i<_options.Length;i++)
            {
                int index=i;
                var button=OwnerTextAction.Create(column,"Option"+i,_options[i],()=>Select(index),0,0,492,56,28);
                var layout=button.gameObject.AddComponent<LayoutElement>();layout.preferredHeight=56;
                button.GetComponentInChildren<Text>().color=i==_selected?OwnerUiTheme.Current.Green:OwnerUiTheme.Current.ActionInk;
