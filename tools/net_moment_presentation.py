"""Qualify host chain totals and accepted milestone delivery on three real local players."""
...
try:
 for index,name in enumerate(['host','scorer','observer']):
  ...
  command=[str(exe),'-batchmode','-screen-width','960','-screen-height','540','-screen-fullscreen','0','-tp-framecap','30','-tp-autostart','3','-tp-map','Eskinita','-tp-profile',profile_name,'-tp-momentseat',str(index),'-tp-momenttrace',str(out/(name+'.csv')),'-logFile',str(out/(name+'.log'))]
  command+=['-tp-momentmode',a.mode]
...
for seat,name in enumerate(['host','scorer','observer']):
 ...
 settled=[r for r in rows if int(r['round'])==1 and int(r['score1'])==705]
 if not settled:errors.append(name+': did not converge to600base+105bonus')
 if max(int(r['first']) for r in rows)!=1 or max(int(r['three']) for r in rows)!=1 or max(int(r['five']) for r in rows)!=1:errors.append(name+': missing or duplicated earned moment')
 after=[r for r in rows if int(r['round'])==2]
 if not after:errors.append(name+': round boundary not received')
 elif any(int(r['moments'])!=3 for r in after):errors.append(name+': stale round moment replayed')
 epochs={int(r['epoch']) for r in rows}
 if len(epochs)!=1 or min(epochs)<=0:errors.append(name+': invalid match epoch')
 measured[name]={'rows':len(rows),'epoch':list(epochs),'settled705':len(settled),'moments':max(int(r['moments']) for r in rows),'charge1':float(rows[-1]['charge1'])}
if len(measured)==3 and len({m['epoch'][0] for m in measured.values()})!=1:errors.append('peers disagree about the host match identity')
if a.old_exe:
 old_log=(out/'old.log').read_text(errors='replace') if (out/'old.log').exists() else ''
 if 'version mismatch' not in old_log.lower() and 'protocol 43' not in old_log.lower():errors.append('old protocol client refusal not witnessed')
if not unchanged:errors.append('shared input preferences changed')
result={'passed':not errors,'errors':errors,'measured':measured,'sharedInputUnchanged':unchanged,'scope':'Three real local Windows peers: physical host chain awards, exact score convergence, unique milestones and old-round rejection. Optional old42client refusal. No remote-input or impairment claim.'}
