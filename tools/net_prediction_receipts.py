...
p=argparse.ArgumentParser();p.add_argument('exe',type=Path);p.add_argument('--out',type=Path,required=True);p.add_argument('--mode',choices=['classic','hero'],default='hero');p.add_argument('--expect-baseline-defects',action='store_true');p.add_argument('--old-exe',type=Path);link_arguments(p);a=p.parse_args()
