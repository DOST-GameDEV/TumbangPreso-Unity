...
def evaluate(folder, scenario):
    ...
    if scenario == "coven" and data["host"]:
        host = data["host"]
...
def main():
    ...
    parser.add_argument("--case", choices=("coven", "rejected"), required=True)
    ...
    def peer(name, route):
        return launch([str(args.exe.resolve()), "-batchmode", "-screen-width", "640",
                       ...
                       "-tp-phaistertrace", str(folder / f"{name}.csv"),
                       "-logFile", str(folder / f"{name}.log")] + route, stdout=subprocess.DEVNULL)
