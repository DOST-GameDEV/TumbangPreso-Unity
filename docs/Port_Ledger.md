...
| `network_manager.gd` | 1413 | `NetSession` + `LobbySession` + `MatchRpc` + `NetAuthority` + `NetBootstrap` + `NetIdentity` (1500) | CONVERTED (N0-N11): NGO+UGS stack locked, NetIdentity wired, LanBeacon converted, Relay host/client + UGS Lobby integrated, Lobby UI converted, spawning/seating/write permissions wired, Ready gate networked, prop/match replication & late-join sync wired, disconnect/AI takeover/seat reclaim wired, Multiplay dedicated server & SQP integrated, multi-process verification probes passing |
