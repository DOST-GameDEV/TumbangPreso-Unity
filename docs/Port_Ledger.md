...
| `network_manager.gd` | 1413 | `NetSession` + `LobbySession` + `MatchRpc` + `NetAuthority` + `NetBootstrap` + `NetIdentity` (1500) | PARTIAL: N0-N10 complete (NGO+UGS locked, NetIdentity wired, LanBeacon converted, Relay host/client + UGS Lobby integrated, Lobby UI converted, spawning/seating/write permissions wired, Ready gate networked, prop/match replication & late-join sync wired, disconnect/AI takeover/seat reclaim wired, Multiplay dedicated server & SQP integrated); N11 Multi-process verification probes pending |
...
| `server_query.gd` | 536 | `ServerQuery.cs` (340) | CONVERTED (N4): legacy VPS pool retired, UGS Lobby discovery with LAN-first code resolution, distinct seated/occupied counts |
