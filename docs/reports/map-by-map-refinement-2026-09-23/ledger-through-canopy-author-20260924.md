...
Masonry is now published083321bf464fb008e8f9cfb5579dfb2c79d9a538, remote verified;
lighting source still50f1fc255. QUAL advanced after owned candidates matched and
were stashed. Canopy author source is drafted for five named upper-growth shapes,
with unchanged lower1.4m/root/colliders; also corrects roof-tinted planter foliage
on the four timber homes. No scene change yet. Current read-only bright baseline
uses the EXISTING PrimaryHomesMaterialReview route in Logs/refine2-garden-before.
After that one capture set, run the author and reuse the same route for comparison;
no new tests/fixture work. Inputs frozen while this baseline runs.
Baseline56408completed1/1in4.801s, inputs/churn preserved then restored. Current
canopy author runs Logs/refine2-garden-v1/author.log; no.cs edits during the run.
The original raw tree meshes are recovered through the existing surface-source
contract before deriving geometry, then the existing surface author is reapplied
only to those trees. Four timber-house planter foliage overlays share one small
green ramp; pots/walls/roots/collision remain. One existing-route after capture next.
