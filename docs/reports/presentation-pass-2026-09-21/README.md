...
Only the credited thrower gets can-hit camera/reticle confirmation. Routine can,
restore, tag and block attribution uses a three-entry side feed. A single moving
can marker communicates down/restoring/protected state; danger requires the real
upright-can condition. Restoration no longer produces a duplicate centre banner.
A brief eligibility cue replaces the continuous red hold. Player numbers/colours
stay fixed across role changes. Force equip and credited throw now enforce the
same ownership rule, and ownerless stock is restricted to explicit training cases.

## Provenance and preservation

All three friend branches were checked as requested. camera-feel-lifecycle-pass
fe9baa16 and abilities-rework dd44acd1 were already ancestors of ASTRAReworks.
feature/slipper-beam-shader f4c819f9 was integrated by fast-forward with original
authorship, including shader, runtime hook, build inclusion and tests. It was
refined in place. Previous active docs are preserved in intake with link targets
relocated. Unrelated pre-existing UI/Inday/probe changes remain unstaged; intake
bytes/hashes are saved in development Logs/presentation-pass-2026-09-21.

## Evidence and limits

- Baseline f4c819f9: SlipperRecallShots 1/1 PASS; tall beam inspected.
- 3694d67c: first exchange fixture 0/2 because the test hit initial restoration
  protection. The legitimate gameplay refusal was retained; the fixture now waits.
- 8d0fead6: related selection executed 15 tests, 14 PASS, 1 remote-smoothing FAIL.
  Ownership, solo training, locator capture and both exchange contracts passed.
- The unchanged f4c819f9 baseline also FAILS that same remote-smoothing test, in
  its return-to-rest assertion. Retain both receipts; not closed or suppressed.
- cde9a55c: final exchange/foreign-use regressions 3/3 PASS, 9.42s.
- Windows build SUCCEEDED, 1156 MB, 161s, normal GameBuilder.BuildWindows. Guard
  82bc2b266774. Artifact in validation workspace Builds/exchange-presentation-v1.
  Exact base is cde9a55cd9f35dcc16128501c0bc67a18e2fe527 plus build-generated inputs
  in build-inputs.json/build-generated-inputs.patch. The only non-whitespace source
  difference is the expected SlipperBeam shader registration, recovered to development.
  EXE SHA256 BA26219C895F07EAE430A96CF0BC31A09BD767905C1FFA957BBFA86D60D2BE59.
- Native --gameplay-only PASS through actual menu/practice entry in both modes,
  movement/sprint/jump/get-up, throw/retrieval/shove/slide, can reset, punch/lunge,
  participant/alternate camera checks and return to menu. Named profile
  presentation-native-20260921; shared input unchanged; owned process17392 retired.
- Inspected low-beacon witness/owner images, both-mode state captures and native
  restoration capture. One early staged image overlapped two actors at the camera;
  positions were separated in the fixture, without modifying production collision.

These are controlled live-route cases, not human acceptance, a freeform full-match
assessment, multiplayer certification or completion of the full prompt. Audio
...
chain rewards, meaningful retained highlights/all-player halftime and full-match
refinement remain open. C4 request/compatibility files remain reserved; no handback
