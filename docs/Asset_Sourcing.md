...
| | |
|---|---|
| Asset | [PlayStation 3 gamepad](https://commons.wikimedia.org/wiki/File:PlayStation_3_gamepad.svg) by Grumbel, Open Clip Art Library via Wikimedia Commons |
| Licence | **CC0 1.0** (Commons template `PD-OpenClipart`). Rule 1 satisfied: free, commercial use allowed, no share-alike. Rule 8 satisfied: CC0 may live in this public repository with its licence |
| Where | `tools/assets/ps3_gamepad_cc0.svg`, its 2400 px rasterisation beside it, and `ps3_gamepad_cc0.LICENSE.txt` |
| Ships as | `Assets/TumbangPreso/Resources/UI/input/pad_diagram_v1.png`, regenerated by `tools/build_controller_diagram.py` |
| Attribution | **Not required.** Recorded anyway; no credits line is owed |

⚠️⚠️ **IT IS RECOLOURED ON THE WAY IN AND THE RAW ART NEVER SHIPS**, which is rule 2 (*"the game
owns the final look. A downloaded prefab is an ingredient, not an art direction"*) and
`CLAUDE.md` § 6.4 together. The source is near-black with **green, magenta, red and purple** face
buttons: magenta is the exact fault § 139.4 records shipping in the settings panel, and the other
three each belong to another job in this palette. The generator maps every pixel onto the warm
ramp by luminance and flattens all four face glyphs to Honey Quartz, **so the four buttons are
told apart by SHAPE**, which is `docs/FUTURE.md` § 16.1's rule and the same call
`tools/build_input_glyphs.py` made about the bought glyph sheets.

⚠️ **THE SEARCH WAS RESTRICTED TO CC0 SOURCES ON PURPOSE.** The ask was to take art off the web;
rule 1 is what decided WHICH art, because an arbitrary image result is almost always somebody's
copyright and this repository is public.
