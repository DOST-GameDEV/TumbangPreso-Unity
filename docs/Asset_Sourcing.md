...
## 2026-09-10 authored improvement assets

The neighborhood OBJ meshes and shared matte materials in `NeighborhoodFinish` are
project-authored geometry, generated by `tools/author_neighborhood_models.py` and
`NeighborhoodFinishAuthor`. Philippine photographs informed proportion/composition;
no reference photographs or advertising pixels are shipped. References and design
choices are recorded in `reports/improvement-2026-09-10/world-direction.md`.

Character additions retain original palettes, source meshes and rig ownership. The
new authored gait/cast channels replace named actions only. First-person arm meshes
are derived from these existing roster models, not separately sourced art.
