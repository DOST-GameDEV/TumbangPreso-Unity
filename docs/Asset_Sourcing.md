...
| | |
|---|---|
| Asset | The "Dualshock 4" sheet from the [8th Gen Console Vector Gamepad Collection](https://opengameart.org/content/8th-gen-console-vector-gamepad-collection) by TheHoodieGuy02, via OpenGameArt |
| Licence | **CC0 1.0**. Rule 1 satisfied outright: free, commercial use, no share-alike, **no attribution owed** |
| Where | `tools/assets/gamepad_cc0.svg`, the pack's own 2048 render cropped beside it, the author's readme, and `gamepad_cc0.LICENSE.txt` |
| Ships as | `Assets/TumbangPreso/Resources/UI/input/pad_diagram_v1.png`, regenerated by `tools/build_controller_diagram.py` |
| Attribution | **Not required.** A courtesy line is in `CreditsContent.CourtesyCredits` anyway, because the author asked for a shoutout in their readme |

⚠️⚠️ **THE TILT IS WHY THIS ONE AND NOT A FLATTER DRAWING.** 🧑 asked for a pad *"tilted like this
ps5 controller"*, and the tilt is not decoration: **on a flat front view the shoulder buttons are
edge-on**, so L1 and L2 are one visible bar and the map has to point two labels at one shape. This
drawing shows the top faces of both shoulders stacked at each corner, so each gets its own anchor.
The author's readme gives the same reason for drawing it: *"what I found on the web doesn't have
back trigger buttons visible."*

⚠️ **IT IS RECOLOURED ON THE WAY IN**, which is rule 2 (*"the game owns the final look"*) and
`CLAUDE.md` § 6.4 together. The body is neutral grey, § 6.4 bans cold grey as flatly as it bans
navy, and the face glyphs are cyan, pink, red and blue. Every pixel maps onto the warm ramp by
luminance and the saturated marks flatten to one honey, so the four buttons are told apart by
SHAPE (`docs/FUTURE.md` § 16.1).

⚠️ **THERE IS NO TRADEMARK IN THIS SOURCE AND BOTH PREVIOUS ONES HAD ONE.** They drew Sony's mark
and the generator carried a disc-erase step, because a licence to reuse a DRAWING is not a licence
to the trademark inside it (`docs/Port_Plan.md` § 8 records the same open item for the IKE
slipper). **If the art is swapped again, look for a mark before assuming there is none.**

⚠️ **WHAT THIS REPLACED, IN ONE DAY**: Grumbel's PlayStation 3 gamepad (CC0, wrong shape), then
Tokyoship's Dualshock 4 Layout (CC BY 3.0, right shape and flat). Both are in this branch's
history. The CC BY one **owed a credits line**; this one does not, and the line moved from
`CcByCredits` to `CourtesyCredits` rather than being deleted.
...
⚠️ **NEITHER LINE ABOVE IS IN THE GAME YET**: both are for assets that have not landed. The
CONTROLLER MAP's pad DID owe one of these for part of 2026-09-04, under CC BY 3.0; § 8.1 records
the CC0 replacement that does not, and the line it left behind is a courtesy row now.
