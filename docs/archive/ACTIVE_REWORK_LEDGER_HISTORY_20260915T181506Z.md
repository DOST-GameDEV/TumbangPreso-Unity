...
## CURRENT nativev30 inspected; participant caption cleanup being qualified

Nativev30 PASSED direct controls in both modes (Logs/close-feedback-native-v30),
PID18372 retired, shared input unchanged. Build1135MB/49s, guard32a94c98685f,
Runtime E0D86CC122CBCABFA5FB29F5BA5B2A19E7C3FE9F266C14FD4EC2484588CEF339.
Paper is now unobtrusive in the actual can view. Capped TAGGED still overlaps the
clock for the first-person tagger, who already has +100TAG HUD feedback. Added
per-view participant suppression, keeping the world caption for others/spectators.
Crucially still spawn/dedupe it normally to preserve the shared random draws.
The simple skip-spawn variant built asv31 (guardb4c7e5326d02) but was not run;
it was replaced before shipping because skipping would change RNG consumption.

Full close-feedback fixturev4 PASSED4/4, guard89a70d1e3991, covering projected
bounds, paper near/far/ortho, impact points and equal random/dedupe paths with
participant visibility. Native driver now captures a real punch tag first from
the participant and then another local character's camera, requiring the caption
object to remain spawned but render only in the latter. This is alternate local
camera coverage, not another network peer. Building
Builds/close-feedback-v32/TumbangPreso.exe, Logs/close-feedback-build-v32.log.
After build, run --gameplay-only with a fresh named profile/output, inspect tag
captures, publish. No imported edits during build/run. The separate ortho-paper check passed1/1,
guardcd8f77120230. Do not repeat unrelated matrices. No original art/models/Net edits.
Owner asleep; no resets/questions/agents. v29 preserved, full queue continues.

## CURRENT close-view feedback3/3 plus cameraPASS; nativev30 building

Verified pushed f5124c2b (v29 functional candidate). Its actual near tag/can captures
show oversized words and foreground paper. New GPU baseline failed2/2:
caption960x196 in960x540 view, one ribbon64x36; guard586f419e5b03.
Implemented: ComicPopup caps close projected size separately per camera, keeps
authored world size at distance, and prepares actual camera culls; thin confetti
uses a scoped Resources shader with screen-size cap/near fade, shared material and
per-renderer tint; ImpactBurst maxParticleSize=.02. No change to counts, lifetimes,
priority, random calls, physics velocity recipes, gameplay or original menu art.

Fixed GPU3/3PASS, guardff9d00efb427: caption near280x58/far100x20/ortho280x58;
paper near12x4, camera-crossing0, far8x2; impact20x20. Existing opposite-camera
caption checkPASS1/1, guard301ca6473984. Renders inspected. Building
Builds/close-feedback-v30/TumbangPreso.exe, Logs/close-feedback-build-v30.log,
exec12854. Next use existing --gameplay-only native path to inspect real tag/can
events. Consider the shader's orthographic paper branch as one final focused
render check. No imported edits during build/native. Do not rerun unrelated suites
or claim native completion yet. Report: reports/close-feedback-2026-09-16.
All new source/artifacts are uncommitted. v29 remains preserved. Owner asleep:
NO resets, optional questions, agents, other chats or paid tools. C4 reserved.
