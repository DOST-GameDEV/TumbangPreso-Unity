...
# Owner rejection and mandatory replacement architecture
The owner explicitly requires COMPLETELY NEW UI builders and says "explicitly not reuse old ui builders". Reskin/reparenting of old builders/hierarchy is superseded. Build empty-root views with new editable native components/theme asset, connecting extracted nonvisual controller/domain behavior. No calls/wrappers of PaperKit/PaperCraft/GodotTheme/legacy screen generation. Preserve old implementations inactive. See newest UI_EXECUTION_STATUS pointer for source palette sheets, icon/actual-portrait priority, rejected partial work and replacement architecture. Broad source palette + quirky uneven/soft print forms must read in rendered screens; no generic cream rectangle dashboard and no defence from token definitions alone.
