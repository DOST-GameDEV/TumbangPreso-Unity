...
**`docs/TODO.md` § 155.** Built, photographed and green. **Two of the three things left are yours
and neither can be measured from a command line.**

**Already done, so do not redo it:** the beam stands on your own loose tsinelas in the colour you
picked in settings, it tapers and fades out before the top, it fades away as you come into reach
so the `[X] Pick up` prompt has the moment to itself, and it turns off entirely when the slipper
highlight setting is Off. It shares every decision with the screen mark rather than keeping a
second copy: whose tsinelas it is, the colour, Off, and when to stand down. `Logs/shots-recall/`
has it from the player's own eyes and from a witness camera beside it.
...
If either is wrong the height and the alphas are one constant each.
...
one effect in the game a player deliberately walks up to and stands under, which is what makes it
worth measuring rather than assuming. That one is engineering, not a judgement call, and it is in
`docs/TODO.md` § 155.5.
